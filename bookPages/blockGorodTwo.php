...
<div class="pageBookBlock">
  ...
  if($_GET['novosti']=="click" or $start=="novosti") {
   $res = $db->query("SELECT * FROM stocks WHERE id > 0 LIMIT 5;");
   
   if($_GET['news']=="click") { $start = "novosti";
    $fog = $_GET['nom'];
  	$res = sqlite_query($db, "SELECT * FROM stocks WHERE id=$fog ;");
	foreach ($res as $array)  
         {
	echo("<div class=\"zagol\">".$array['zag']."</div><div class=\"text\">".stripcslashes($array['text'])."</div>");
         }
		 }   
   else{
   foreach ($res as $array)  
